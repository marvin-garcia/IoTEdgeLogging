# Azure IoT ELMS (Edge Logging & Monitoring Solution)

Whenever there are an IoT edge devices involved, there will be software modules running on it generating useful logging data and metrics that can help monitor performance and provide feedback to teams, whether they are developers, data engineers, product vendors, etc. The purpose of this article is to provide a solution to capture logs and metrics generated by modules at the edge and store them in Azure Log Analytics, where they can later be queried and correlated.
...
3. If the monitoring option is configured to upload metrics as IoT messages, an event hubs namespace and event hub instance will be required to route metric messages from IoT Hub to the Azure function **CollectMetrics**, that will then send them to Log Analytics.
